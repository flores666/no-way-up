...
public sealed partial class PlayerWeaponController2D : Node2D, INoiseEmitter2D
{
	...
	private bool _firePressedPending;
	...
	public override void _Ready()
	{
		...
		if (!_reloadTimer.OneShot)
		{
			throw new InvalidOperationException(
				$"{nameof(PlayerWeaponController2D)} on '{Name}' requires a one-shot reload timer.");
		}
	...
	public override void _ExitTree()
	{
		...
		_isFireHeld = false;
		_firePressedPending = false;
		if (GodotObject.IsInstanceValid(_weaponFx))
		{
			_weaponFx.StopAll();
		}
		SetAiming(false);
	...
	{
		if (!_isInitialized || !_isCombatInputEnabled)
		{
			_firePressedPending = false;
			return;
		}

		// Execute the initial trigger press on the physics tick. Player movement and
		// AimPivot are updated by PlayerController2D in physics as well, so the first
		// shot never samples a transform from an input/render frame while moving.
		if (_firePressedPending)
		{
			_firePressedPending = false;
			FirearmShotResult initialResult = TryFire();
			if (!initialResult.Success &&
				State.Definition.FireMode == FirearmFireMode.Automatic)
			{
				double blockedAtSeconds = Time.GetTicksMsec() / 1000.0;
				_nextAutomaticAttemptAllowedAtSeconds =
					blockedAtSeconds + State.Definition.FireIntervalSeconds;
			}
	...
	private void PerformHitscan(ValidatedShotPath shotPath)
	{
		...
		Vector2 visualEnd = shotPath.RayEnd;
		bool hitObstacle = false;
		if (hit.Count > 0)
		{
			visualEnd = hit["position"].AsVector2();
			GodotObject? collider = hit["collider"].AsGodotObject();
			hitObstacle = !TryApplyHitDamage(collider);
		}

		_weaponFx.PlayShot(
			shotPath.RayStart,
			visualEnd,
			hitObstacle ? WeaponImpactFxKind.Obstacle : WeaponImpactFxKind.None);
	}
	...
	private bool TryResolveValidatedShotPath(out ValidatedShotPath shotPath)
	{
		...
		// visual projectile and hit detection always match the equipped weapon length.
	...
	private bool TryApplyHitDamage(GodotObject? collider)
	{
		...
		if (ReferenceEquals(healthOwner, Player) ||
